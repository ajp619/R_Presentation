...
My presentation tonight was inspired by a lecture from the Coursera course "Data Analytics"" I took from Professor Jeff Leek from Johns Hopkins. In it he demonstrated the effects of population sampling and confidence intervals in a way that I found very intuitive and accessible. In the process, he also demonstrated some very interesting techniques in R.
...
Let's take a look at what we get with a linear model. Specifically, I want to play with different plotting options and get a feel for what coefficients, fitted.values, and residuals are:
...
```{r}
# to run all of a markdown file ctrl+alt+r

# to run current chunk ctrl+alt+c
